        ...
        axios.get('/dashboard/admins/data', {
            ...
            if (res.data.data.length === 0) {
                ...
                const roles = (admin.roles || []).map(r => r.name).join(', ') || '—';
                    ...
                    let actions = '';
                    if (window.adminPermissions?.update) {
                        actions += `<button onclick="AdminsManager.editAdmin(${admin.id})" 
                    ...
                    if (window.adminPermissions?.delete) {
                        actions += `<button onclick="AdminsManager.deleteAdmin(${admin.id})" 
                    ...
                    actions = actions || '<span class="text-slate-400 text-xs">لا صلاحيات</span>';
