<x-dashboard.layout title="إدارة المستخدمين">
    <!-- غطاء تحميل عام -->
    ...
    <!-- نافذة المستخدم -->
    <div id="user-modal" class="fixed inset-0 bg-black bg-opacity-50 z-40 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-6 border-b border-slate-200">
                <h2 id="modal-title" class="text-2xl font-bold text-slate-800">إضافة مستخدم جديد</h2>
                <button onclick="UsersManager.closeModal()" class="text-slate-400 hover:text-slate-600 transition">
            ...
            <form id="user-form" class="p-6">
                ...
                <div class="flex gap-3 mt-8 pt-6 border-t border-slate-200">
                    ...
                    <button type="button" onclick="UsersManager.closeModal()" class="flex-1 px-6 py-3 bg-slate-200 hover:bg-slate-300 text-slate-800 rounded-lg font-medium transition">
                        إلغاء
                    </button>
                </div>
            </form>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
        <div class="flex border-b border-slate-200">
            ...
            <button class="tab-btn flex-1 px-6 py-3 text-center font-medium active border-b-2 border-blue-500 text-slate-900" data-tab="users">المستخدمون</button>
        </div>
        ...
        <div id="tab-stats" class="tab-content p-6 hidden">
            ...
            <div class="bg-slate-50 rounded-xl p-6 shadow-sm border border-slate-200">
                <h3 class="text-lg font-semibold mb-6 text-slate-800">سجل إضافة/إنضمام المستخدمين علي مدار آخر 30 يوما</h3>
                <canvas id="usersChart" class="w-full" style="max-height: 400px;"></canvas>
            </div>
        </div>

        <div id="tab-users" class="tab-content p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-slate-800">قائمة المستخدمين</h2>
                <button onclick="UsersManager.openModal()" class="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition shadow-sm">
            ...
            <div class="mb-6 space-y-4">
                <div class="flex flex-wrap gap-4">
                    ...
                    <button onclick="UsersManager.loadUsers()" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition">بحث</button>

                    <button onclick="UsersManager.resetFilters()" class="px-6 py-3 bg-slate-200 hover:bg-slate-300 text-slate-800 rounded-lg font-medium transition">إعادة الضبط</button>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                ...
                <table id="users-table" class="w-full min-w-[720px] text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-6 py-4 text-right font-semibold cursor-pointer hover:bg-slate-100 transition" onclick="UsersManager.sortColumn('name')">الاسم</th>
                            <th class="px-6 py-4 text-right font-semibold cursor-pointer hover:bg-slate-100 transition" onclick="UsersManager.sortColumn('email')">البريد الإلكتروني</th>
                            <th class="px-6 py-4 text-right font-semibold">رقم الهاتف</th>
                            <th class="px-6 py-4 text-right font-semibold cursor-pointer hover:bg-slate-100 transition" onclick="UsersManager.sortColumn('created_at')">تاريخ الإنشاء</th>
            ...
            <div class="flex items-center justify-between mt-6">
                ...
                <div class="flex gap-3">
                    <button id="prev-btn" onclick="UsersManager.changePage(UsersManager.prevPage)" disabled class="px-5 py-3 border border-slate-300 rounded-lg text-sm disabled:opacity-50 hover:bg-slate-50 transition">السابق</button>
                    <button id="next-btn" onclick="UsersManager.changePage(UsersManager.nextPage)" disabled class="px-5 py-3 border border-slate-300 rounded-lg text-sm disabled:opacity-50 hover:bg-slate-50 transition">التالي</button>
                </div>
            </div>
        </div>
    </div>
